...
const CREDENTIALS = { email: 'anshika@gmail.com', password: 'Iamking@000' };
...
test('Assgnment 3 Test1 — Banner IS visible when 6 events are returned', async ({ page }) => {
...
test('Assgnment 3 Test2 — Banner is NOT visible when 4 events are returned', async ({ page }) => {

    await page.route("**/api/events**",
        async route => {
            //intercepting response - API response ->{playwright fakeresponse } -> Browser -> render data on front en
            route.fulfill(
                {
                    status: 200,
                    contentType: 'application/json',
